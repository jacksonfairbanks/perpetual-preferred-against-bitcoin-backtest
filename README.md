...
Daily-cadence sensitivity (paper §5.1 cadence footnote) only needs the first two:
...
└── outputs/
    ...
    ├── regime_descriptors.csv               one row per regime: peak/trough/recovery dates and required-BCR brackets
    ...
    └── daily/                               daily-cadence sister run (paper §5.1 cadence footnote)
        ├── manifest.json
        ├── regime_summary.csv
