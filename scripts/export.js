    ...
    return {
        ...
        failure_definitions: {
            ...
            terminal_state_semantics: {
                ...
                censored:       "scenario window ended with no failure, no terminal liquidity, and no recovery -- the scenario cleared primary failure but had not returned to its starting BCR by the regime window cutoff (paper §6.3, Appendix D 'censored scenario'); reported as such, not classified as failure"
            }
        },

        metrics: {
            bitcoin_coverage_ratio: {
                definition: "BTC reserve value / annual dividend obligation, evaluated daily. The paper's term for the metric; 'BCR' is the abbreviation used throughout.",
                formula: "bitcoin_coverage_ratio = (btc_held * price_usd) / annual_obligation_usd",
                aliases: ["bcr"],
                naming_note: "Earlier drafts called this 'balance_sheet_coverage_ratio' on the theory that it might one day include a cash component. Since this paper models a pure-BTC reserve, 'Bitcoin Coverage Ratio' is the accurate term."
            },
            cash_coverage_ratio: {
        ...
        estimators: {
            ...
            dividend_convention: {
                ...
                annualization: "annual_obligation_usd = btc_reserve_initial_usd / starting_bcr (equivalently btc_reserve_initial_usd * preferred_to_reserve * div_rate with preferred_to_reserve = 1 / (starting_bcr * div_rate))",
                monthly_draw: "exact monthly payment = annual_obligation_usd / 12; no accrual/payment distinction",
                note: "Since BCR depends only on the product preferred_to_reserve * div_rate (with OPEX = 0), starting_bcr fully parameterizes the obligation schedule. div_rate is carried for dollar-sanity columns only."
            }
        },
        ...
        engine_config: {
            ...
            reproduction_command: CADENCE === 'daily'
                ? "from repo root: node scripts/preprocess-bitstamp.js && node scripts/export.js --cadence=daily"
                : "from repo root: node scripts/preprocess-bitstamp.js && node scripts/export.js && node scripts/build-surface.js && node scripts/build-surface-grid-validation.js"
        },
        ...
        sweep_grid: {
            ...
            tenth_step_densifier_zones: [
                { range: [1.0, 2.0], motivation: "Terminal-liquidity transition zone for 2020 COVID" },
                { range: [2.0, 3.0], motivation: "BCR<1x transition zone for 2020 COVID; terminal-liquidity transition zone for 2013 April flash crash" },
                ...
                { range: [7.0, 8.0], motivation: "BCR<1x transition zone for 2022 Fed tightening; terminal-liquidity transition zone for 2011" },
                ...
                { range: [13.0, 20.0], motivation: "BCR<1x transition zone for 2011 (deepest drawdown => highest required starting BCR)" }
            ],
            densifier_rationale: "Integer grid resolves required starting BCRs to within 1 BCR unit everywhere; 0.1-step densifier zones resolve each regime's BCR<1x and terminal-liquidity boundaries to 0.1 BCR units. Both the primary (BCR<1x) and terminal-liquidity required starting BCRs are resolved to 0.1 for every regime."
        },
        ...
        outputs: {
            ...
            'regime_summary.csv':      { rows: ctx.summaryRowCount,    description: "One row per scenario. Terminal state + event dates + required-BCR brackets + integral/time-weighted metrics." },
            'regime_descriptors.csv':  { rows: ctx.descriptorRowCount, description: "One row per regime. Peak/trough/recovery anchors, drawdown_shape_ratio, required-BCR brackets, selection_rationale." }
        },
